# KaChing Walk-Forward Backtester — Implementation Plan
...
- **Cell 5**: Long put helpers
  ...
  - `price_long_put_at_expiry(long_put_row, raw_df)` — looks up `underlying_close` on `expiration` date from raw data, returns `max(0, strike - close)`
...
- **Cell 6**: `run_walkforward_single(ticker, entry_dow, selection_method)` — core function
  ...
  - Note: pass `raw_df` as-is to both filter functions — `filter_weekly_put_options` rebinds locally, won't clobber the caller's reference
...
## Verification Steps
1. Run cells 1-3 — universe loads cleanly, shows ~30 A-tier tickers
